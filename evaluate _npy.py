...
'''
    ...
    with torch.no_grad():
        print("\n==> Start evaluating ...")
        lr = torch.tensor(np.load('input_lr.npy')).permute(0,3,1,2)
        pan = torch.tensor(np.load('input_pan.npy')).unsqueeze(0)
        target = torch.tensor(np.load('targ.npy')).permute(0,3,1,2)

        pan, mslr, mshr = pan.to(device), lr.to(device), target.to(device)
        ...
        print(f'psnr= {val_metric["psnr"]:.4f}, ssim={val_metric["ssim"]:.4f}')
